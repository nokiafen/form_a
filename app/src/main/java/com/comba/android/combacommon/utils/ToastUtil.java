...
public class ToastUtil {
	 ...
	 */
	static int dimen = DensityUtil.dip2px(PlatFormApplication.getContext(), 160);

	public static void showToastWithAlertPic(String msg) {
		if (!TextUtils.isEmpty(msg) && !msg.equals(showingText)) {
			toast = Toast.makeText(PlatFormApplication.getContext(), msg, Toast.LENGTH_SHORT);
			toast.setGravity(Gravity.CENTER, 0, 0);
			View toastView = View.inflate(PlatFormApplication.getContext(),
					R.layout.toast_custom_view, null);
			LinearLayout llToastView = (LinearLayout) toastView
					.findViewById(R.id.llToastView);
			if (msg.length() <= 6) {
				LayoutParams params = (LayoutParams) llToastView
						.getLayoutParams();
				params.width = dimen;
				params.height = dimen;
				...
				LayoutParams params = new LayoutParams(
						LayoutParams.WRAP_CONTENT, dimen);
				llToastView.setLayoutParams(params);
			}
	...
	public static void showToastWithAlertPic(Context context,int resId) {
		showToastWithAlertPic(context.getResources()
				.getString(resId));
	}
	...
	public static void showToastWithOkPic(String msg) {
		if (!TextUtils.isEmpty(msg) && !msg.equals(showingText)) {
			...
			((TextView) toastView.findViewById(R.id.tvToastText)).setText(msg);
			((ImageView) toastView.findViewById(R.id.ivToastIcon))
					.setImageResource(R.drawable.ic_tip_ok);
			toast.setView(toastView);
			toast.show();
			showingText = msg;
			newCountDownTimer(2000);
	...
	public static void showToastWithOkPic(int resId) {
		showToastWithOkPic(PlatFormApplication.getContext().getResources().getString(resId));
	}
}
